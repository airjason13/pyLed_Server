            ...
            ff = ffmpy.FFmpeg(
                ...
                outputs={
                    udp_sink: ["-preset", "ultrafast", "-vcodec", "libx264", '-filter_complex', filter_params,
                               "-g", "60", "-r", target_fps, "-f", "h264", "-pix_fmt", "yuv420p", "-localaddr", localaddr],
                    #udp_sink: ["-preset", "ultrafast", "-vcodec", "libx264", '-filter_complex', filter_params,
...
def neo_ffmpy_cast_video_h264(video_path, cast_dst, brightness, contrast, red_bias, green_bias, blue_bias, width=80, height=96):
    ...
    if platform.machine() in ('arm', 'arm64', 'aarch64'):
        ...
        video_encoder = "libx264"
    for i in cast_dst:
        output[i] = ["-vcodec", video_encoder, '-filter_complex', filter_params, "-b:v", "2000k", "-r", target_fps, "-f",
...
def neo_ffmpy_cast_cms(video_path, cast_dst, window_width, window_height, window_x, window_y,
    ...
    ff = ffmpy.FFmpeg(
        ...
        inputs={
            ...
            # video_path: ["-f", "x11grab", "-video_size", window_size_params, "-r", target_fps]
        },
        outputs={
            udp_sink: ["-vcodec", video_encoder, '-filter_complex', filter_params, "-b:v", "2000k", "-r", target_fps,"-f",
                       "h264", "-pix_fmt", "yuv420p", "-bufsize", "50","-localaddr", localaddr]
        },
    )
